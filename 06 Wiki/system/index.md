# Wiki Index — Navigation Map

> **Last updated:** 2026-05-09
> **Total pages:** 28
> **How to use:** Read this first every session. Find relevant pages, then drill in. Update after every ingest or new page creation.

---

## System

| Page | Summary |
|---|---|
| [[system/WIKI-SCHEMA.md]] | Constitution — operating rules, workflows, naming conventions |
| [[system/index.md]] | This file — content map and navigation |
| [[system/log.md]] | Append-only timeline of all wiki activity |

---

## FBA

| Page | Summary |
|---|---|
| [[wiki/fba/fba-research-overview.md]] | **START HERE for FBA** — master rankings, all niches compared, priority action queue |
| [[wiki/fba/niche-silicone-splatter-screen.md]] | ❌ PASS — real H10 revenue $1,277/mo total on silicone keyword; market too small |
| [[wiki/fba/niche-cable-management.md]] | 72/100 🟡 Investigate More — keyword dominated by raceways; box sub-niche real, one H10 Xray resolves it |
| [[wiki/fba/niche-padded-lifting-straps.md]] | 85/100 🟡 Investigate More — needs sub-keyword H10 Xray before committing |
| [[wiki/fba/niche-fabric-resistance-bands.md]] | 66/100 🟡 Caution — 950 avg reviews, only via non-slip long-tail variant |
| [[wiki/fba/niche-dog-travel-accessories.md]] | 51/100 🔶 Caution — 6,908 avg reviews, revisit in 18 months |
| [[wiki/fba/niche-bamboo-kitchen-utensils.md]] | 37/100 ❌ Pass — satisfied buyers, no differentiation, small market |
| [[wiki/fba/fba-listing-optimisation.md]] | Title rules, bullet structure, image sequence, A+ Content, backend keywords — full listing build guide for Voya |
| [[wiki/fba/fba-ranking-factors.md]] | Amazon A10 ranking signals — sales velocity, CTR, CVR, keyword relevance, FBA status, stock rate |
| [[wiki/fba/fba-compliance-suppression.md]] | Suppression risk table, all prohibited content, new ASIN rules, re-branding rules, GTIN requirements |
| [[wiki/fba/fba-image-requirements.md]] | Technical specs, main image rules (white bg, 85% fill, no text), compliance checklist, Voya-specific notes |

---

## FBA Launches

*No active launches yet — hub gets created when a product is confirmed post-H10 validation.*

---

## Business

*No pages yet.*

---

## Finance

| Page | Summary |
|---|---|
| [[wiki/finance/stock-trading-overview.md]] | Portfolio ~$20k, AI/power infrastructure thesis, ETF-first strategy, individual stock rule formalised, EXR/ERA decisions documented. Updated 2026-05-06. |

---

## Fitness

*No pages yet.*

---

## Cars

*No pages yet.*

---

## Personal

*No pages yet.*

---

## Research

| Page | Summary |
|---|---|
| [[wiki/research/ai-2027-scenario.md]] | Full synthesis of the AI 2027 scenario — timeline, milestone table, alignment progression, what it means for Dave |

---

## People

| Page | Summary |
|---|---|
| [[wiki/people/josh.md]] | ⚠️ PLACEHOLDER — Best mate and FBA/trading partner. Key details (stake, capital split, trading arrangement) not yet confirmed. |

---

## Concepts

| Page | Summary |
|---|---|
| [[wiki/concepts/fba-scoring-framework.md]] | Full FBA scoring rubric — pre-filter, 5-category scoring (100pts), H10 research sequence, verdict thresholds |
| [[wiki/concepts/llm-wiki-pattern.md]] | The LLM Wiki pattern — raw sources, wiki layer, schema, compounding knowledge base |
| [[wiki/concepts/ai-capability-milestones.md]] | SC / SAR / SIAR / ASI definitions, forecast dates, AI R&D progress multiplier explained |
| [[wiki/concepts/ai-alignment-risk.md]] | Alignment faking, scheming, sandbagging — why smarter AIs are harder to keep aligned, and why the problem doesn't get solved |

---

## Sources

| Page | Date ingested | Source type | Summary |
|---|---|---|---|
| [[wiki/sources/src — LLM Wiki Pattern.md]] | 2026-05-03 | Article / idea file | Pattern for LLM-maintained persistent wiki — not RAG |
| [[wiki/sources/src — Gym Accessories Research.md]] | 2026-05-03 | FBA research (simulated) | Gym accessories niche — lifting straps (85), resistance bands (66), wrist wraps |
| [[wiki/sources/src — Silicone Splatter Screen Research.md]] | 2026-05-03 | FBA research (simulated) | Silicone splatter screen — 93/100 Green Light |
| [[wiki/sources/src — Three Niche Comparison.md]] | 2026-05-03 | FBA research (simulated) | Three niches compared — cable management (88), dog accessories (51), bamboo utensils (37) |
| [[wiki/sources/src — AI 2027.md]] | 2026-05-03 | Scenario forecast (ai-2027.com) | AI development scenario mid-2025 to late-2027 — AGI by July 2027, ASI by Dec 2027 in race ending |
| [[wiki/sources/src — Amazon Product Detail Page Rules.md]] | 2026-05-03 | Official Amazon policy | Amazon Seller Central listing rules — titles, images, compliance, ASIN creation, Brand Registry |
| [[wiki/sources/src — Amazon Product Image Guide.md]] | 2026-05-06 | Official Amazon policy | G1881 — full image technical specs, main image rules, compliance requirements |
| [[wiki/sources/src — H10 Silicone Splatter Screen.md]] | 2026-05-09 | Real H10 Xray export | 3 keywords — silicone splatter screen, splatter screen for frying pan, splatter guard. Revenue far lower than simulated. |

---

## Priority Actions (Current)

1. **H10 Xray — "padded lifting straps" and "figure 8 lifting straps"** — sub-keyword avg reviews is the decision point. Under 200 → Green Light. 400+ → move on. Leading candidate, 46% margin.
2. **H10 Xray — "cable management box" and "desk cable organizer"** — box-specific only (not "cable hider"). 15 mins to get a real verdict on this niche. Second candidate.
3. ⛔ Do NOT order samples until at least one niche confirmed with real H10 review + revenue data
4. Silicone splatter screen — DEAD. No further action.

---

## Suggested Next Ingests

- Real Helium 10 Xray exports when pulled — ingest as source files to update niche pages
- Alibaba supplier quotes and sample evaluations
- ~~Amazon image requirements deep-dive (G1881)~~ ✅ Ingested — see `wiki/fba/fba-image-requirements.md`
- Amazon Prohibited Product Claims policy (G202024200) — referenced; worth a dedicated compliance page
- Any trading framework or stock research Dave and Josh are developing
